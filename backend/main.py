...
from services.rag_service import vector_search, hybrid_search, format_retrieved_chunks_for_prompt
...
async def health_check():
    return {"status": "healthy", "services": {"database": True, "llm": bool(LLM_API_KEY)}}
...
async def chat(request: QueryRequest):
    ...
    if query_type == "DOCUMENT_QUERY":
        ...
        if not chunks:
            response = "I could not find any relevant information in the clinic documents."
            sources = []
            source_chunks = []
        else:
            retrieved_chunks = format_retrieved_chunks_for_prompt(chunks)
