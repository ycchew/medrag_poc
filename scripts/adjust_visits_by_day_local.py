...
Usage: python scripts/adjust_visits_by_day_local.py
"""
...
def adjust_visits_by_day():
    ...
    try:
        ...
        boost_days = [(0, "Sunday"), (1, "Monday"), (5, "Friday")]
        ...
        avg_typical_final = sum([final_counts.get(d, 0) for d in typical_days]) / len(
            typical_days
        )
        for day_num, day_name in boost_days:
            count = final_counts.get(day_num, 0)
            boost_pct = (
                ((count / avg_typical_final) - 1) * 100 if avg_typical_final > 0 else 0
            )
...
if __name__ == "__main__":
    print("=" * 60)
    print("Adjust Visits by Day (Local)")
    print("=" * 60)
    print()
